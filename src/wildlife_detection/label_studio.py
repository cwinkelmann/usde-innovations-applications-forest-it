...
class _JWTSession(requests.Session):
    """Session that auto-refreshes expired JWT access tokens."""
    ...
    def _refresh_access(self):
        ...
        r.raise_for_status()
        self.headers.update({
            "Authorization": f"Bearer {r.json()['access']}"
        })
...
def make_session(token: str, url: str = "http://localhost:8080") -> requests.Session:
    ...
    short-lived access tokens, and re-refreshed on 401 expiry.
    """
    if token.startswith("eyJ"):
        return _JWTSession(token, url)
...
def upload_image(session: requests.Session, url: str, project_id: int, image_path: Path) -> dict:
    ...
    # LS 1.23+: find the task by matching the filename in data.image or file_upload.
    # Task creation may be async, so retry briefly.
    import time
    file_upload_ids = data.get("file_upload_ids", [])
    if file_upload_ids:
        ...
        for _attempt in range(5):
            ...
            for t in task_list:
                # Match on file_upload field or data.image URL
